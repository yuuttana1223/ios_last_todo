...
class CompleteViewController: UIViewController {
    ...
    let realm = try! Realm()
    var todos = try! Realm().objects(Todo.self).filter("isDone == true").sorted(byKeyPath: "deadline", ascending: false)
    var nowTodos = try! Realm().objects(Todo.self).filter("isDone == true").sorted(byKeyPath: "deadline", ascending: false)
