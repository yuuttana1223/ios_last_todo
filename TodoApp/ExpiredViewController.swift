...
class ExpiredViewController: UIViewController {
    ...
    let realm = try! Realm()
    var todos = try! Realm().objects(Todo.self).filter("deadline < %@", Date()).sorted(byKeyPath: "deadline", ascending: false)
    var nowTodos = try! Realm().objects(Todo.self).filter("deadline < %@", Date()).sorted(byKeyPath: "deadline", ascending: false)
...
extension ExpiredViewController: UISearchBarDelegate {
